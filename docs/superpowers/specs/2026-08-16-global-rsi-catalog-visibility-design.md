...
The result was reproduced in fresh read-only Codex CLI processes using versions
...
3. A fresh read-only Codex process can identify the installed RSI skill from
   its catalog. The probe must not call the provider CLI or mutation paths.
...
7. The full package, deployment, rollout, forward, adversarial, and repository
   test suite passes; the skill validator, learning-ledger validator, link,
   permission, placeholder, compile, and diff checks pass.
...
during the visibility-only probe is a failure.
...
3. run a fresh catalog-visibility probe;
